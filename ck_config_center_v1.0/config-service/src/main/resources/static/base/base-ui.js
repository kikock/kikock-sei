...
(function ($) {
    $.extend({
            ...
            exportExcel: function (formId) {
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function () {
                    ...
                    $.post(table.options.exportUrl, $("#" + currentId).serializeArray(), function (result) {
                        if (result.operateStatusEnum === status_code.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                        } else if (result.operateStatusEnum === status_code.WARNING) {
        ...
        modal: {
            ...
            icon: function (type) {
                var icon = "";
                if (type == status_code.WARNING) {
                    icon = 0;
                } else if (type == status_code.SUCCESS) {
                    icon = 1;
                } else if (type == status_code.FAIL) {
                    icon = 2;
                } else {
                    icon = 3;
                }
            ...
            msgError: function (content) {
                $.modal.msg(content, status_code.FAIL);
            },
            // 成功消息
            msgSuccess: function (content) {
                $.modal.msg(content, status_code.SUCCESS);
            },
            // 警告消息
            msgWarning: function (content) {
                $.modal.msg(content, status_code.WARNING);
            },
            // 弹出提示
            ...
            alertError: function (content) {
                $.modal.alert(content, status_code.FAIL);
            },
            // 成功提示
            alertSuccess: function (content) {
                $.modal.alert(content, status_code.SUCCESS);
            },
            // 警告提示
            alertWarning: function (content) {
                $.modal.alert(content, status_code.WARNING);
            },
            // 关闭窗体
            close: function () {
            ...
            open: function (title, url, width, height, callback) {
                ...
                if ($.common.isNotEmpty(table.options.popModalWidth)) {
                    width =table.options.popModalWidth
                }
                if ($.common.isNotEmpty(table.options.popModalHeight)) {
                    height =table.options.popModalHeight
                }
                if ($.common.isEmpty(height)) {
        ...
        operate: {
            ...
            ajaxSuccess: function (result) {
                if (result.operateStatusEnum === status_code.SUCCESS && table.options.type == table_type.bootstrapTable) {
                    $.modal.msgSuccess(result.msg);
                    $.table.refresh();
                } else if (result.operateStatusEnum === status_code.SUCCESS && table.options.type == table_type.bootstrapTreeTable) {
                    $.modal.msgSuccess(result.msg);
                    $.treeTable.refresh();
                } else   if (result.operateStatusEnum === status_code.WARNING) {
                    $.modal.alertWarning(result.msg)
            ...
            saveSuccess: function (result) {
                if (result.operateStatusEnum === status_code.SUCCESS) {
                    $.modal.msgReload("保存成功,正在刷新数据请稍后……", status_code.SUCCESS);
                } else   if (result.operateStatusEnum === status_code.WARNING) {
                    $.modal.alertWarning(result.msg)
                } else {
                    $.modal.alertError(result.msg);
            ...
            successTabCallback: function (result) {
                if (result.operateStatusEnum === status_code.SUCCESS) {
                    ...
                    var $contentWindow = $('.Home_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
