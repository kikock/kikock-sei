...
public abstract class KCMPEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered, DisposableBean {
    ...
    protected void initKCMPEnvironment(ConfigurableEnvironment environment, Properties properties) {
        //是否是加载本地配置文件配置
        boolean isConfigFile = Boolean.parseBoolean(environment.getProperty(ConfigConstants.IS_CONFIG_FILE));
        if(isConfigFile){
            System.out.println("只加载本地配置");
        }else {
            System.out.println("加载zookeeper配置");
        }
        //系统环境变量
    ...
    private Map<String, Map<String, String>> getZookeeperData(String appId, CuratorFramework client) {
        try {
            //应用服务的配置节点
            ...
            // String jsonData = new String(client.getData().forPath("/" + appId), "UTF-8");
            if (StringUtils.isNotBlank(jsonData)) {
                log.info("receive nodeChanged event：" + jsonData);
                System.out.println("配置中心配置参数(读取zk数据)：" + jsonData);
