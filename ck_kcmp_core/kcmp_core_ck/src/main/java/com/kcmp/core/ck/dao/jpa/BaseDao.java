package com.kcmp.core.ck.dao.jpa;
 ...
 * <br>
 * <i>And</i> --- 等价于 SQL 中的 and 关键字，比如 findByUserNameAndPassword(String userName, String pwd)；<br>
 * <i>Or</i> --- 等价于 SQL 中的 or 关键字，比如 findByUserNameOrAddress(String userName, String addr)；<br>
 ...
 * <i>Like</i> --- 等价于 SQL 中的 "like"，比如 findByUserNameLike(String userName)；<br>
 * <i>NotLike</i> --- 等价于 SQL 中的 "not like"，比如 findByUserNameNotLike(String userName)；<br>
 * <i>OrderBy</i> --- 等价于 SQL 中的 "order by"，比如 findByUserNameOrderBySalaryAsc(String userName)；<br>
 * <i>Not</i> --- 等价于 SQL 中的 "！ ="，比如 findByUserNameNot(String userName)；<br>
 * <i>In</i> --- 等价于 SQL 中的 "in"，比如 findByUserNameIn(Collection<String> userNameList) ，方法的参数可以是 Collection 类型，也可以是数组或者不定长参数；<br>
 * <i>NotIn</i> --- 等价于 SQL 中的 "not in"，比如 findByUserNameNotIn(Collection<String> userNameList) ，方法的参数可以是 Collection 类型，也可以是数组或者不定长参数；
...
public interface BaseDao<T extends Persistable & Serializable, ID extends Serializable> extends JpaRepository<T, ID>, JpaSpecificationExecutor<T> {
     ...
     * @return 是否已经存在
     */
    boolean isExistsByCode(String code, String id);
     ...
     * @return 是否已经存在
     */
    boolean isExistsByCode(String code, String id,String tenantCode);
}
