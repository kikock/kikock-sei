...
public class TaskEntityManager extends AbstractManager {
   ...
   * @param cascade
   */
  public void deleteRuningTask(String taskId, String deleteReason, boolean cascade) {
	    TaskEntity task = Context
	      .getCommandContext()
	      .getTaskEntityManager()
	      .findTaskById(taskId);
	    ...
	    if (task!=null) {
	      ...
	      //
	      Context
	      .getCommandContext().getVariableInstanceEntityManager().deleteVariableInstanceByTask(task);

	      Context
	      .getCommandContext().getHistoricVariableInstanceEntityManager().deleteHistoricVariableInstancesByTaskId(task.getId());
	      
	      deleteTask(task, reason, cascade);
	      if(task.getExecutionId() != null) {
	    	  try{
	    		  if(task.getExecution()!=null){
	    			  List<TaskEntity> resultTaskEntity = this.findTasksByExecutionId(task.getExecution().getId());
	    			  if(resultTaskEntity!=null && resultTaskEntity.size()==1){
//	    				  ExecutionEntity pExecutionEntity=  task.getExecution().getParent();
//	    				  if(pExecutionEntity!=null){
//		    				  List<VariableInstanceEntity> vEntities =   Context
//		    	    			      .getCommandContext().getVariableInstanceEntityManager().findVariableInstancesByExecutionId(pExecutionEntity.getId());
//		    	    				 if( vEntities !=null && !vEntities.isEmpty()){
//		    	    					 for(VariableInstanceEntity v:vEntities){
//		    	    					 Context
//		    	    				      .getCommandContext().getVariableInstanceEntityManager().delete(v);
//		    	    					 }
//		    	    				 }
//		    	    				 Context
//		   		    		      .getCommandContext().getExecutionEntityManager().delete(pExecutionEntity);
//	    				  }
	    				  List<VariableInstanceEntity> vEntities =   Context
	    			      .getCommandContext().getVariableInstanceEntityManager().findVariableInstancesByExecutionId(task.getExecution().getId());
	    				 if( vEntities !=null && !vEntities.isEmpty()){
	    					 for(VariableInstanceEntity v:vEntities){
	    					 Context
	    				      .getCommandContext().getVariableInstanceEntityManager().delete(v);
		    	    	  ...
		    	    	  Context
		    		      .getCommandContext().getExecutionEntityManager().delete(task.getExecution());
	    			  }
