...
public class ExecutionEntityManager extends AbstractManager {
   ...
   */
  public void updateExecution(ExecutionEntity executionEntity) {
//	    CommandContext commandContext = Context.getCommandContext();
//	    Date expirationTime = commandContext.getProcessEngineConfiguration().getClock().getCurrentTime();
//	    int lockMillis = commandContext.getProcessEngineConfiguration().getAsyncExecutor().getAsyncJobLockTimeInMillis();
//	    GregorianCalendar lockCal = new GregorianCalendar();
...
//	    params.put("expirationTime", expirationTime);
	    ...
	    int result = getDbSqlSession().update("updateExecution", executionEntity);
	    if (result == 0) {
	    	throw new ActivitiOptimisticLockingException("Could not lock process instance");
	    }
	  }
  ...
  public void   deleteExecution(ExecutionEntity executionEntity) {
	  CommandContext commandContext = Context.getCommandContext();
	  List<VariableInstanceEntity> vEntities =   commandContext.getVariableInstanceEntityManager().findVariableInstancesByExecutionId(executionEntity.getId());
			 if( vEntities !=null && !vEntities.isEmpty()){
	    	  ...
	    	  Context
		      .getCommandContext().getExecutionEntityManager().delete(executionEntity);
	  getDbSqlSession().delete(executionEntity);
	  }
}
